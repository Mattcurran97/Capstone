...
def open_db(filename: str) -> tuple[sq.Connection, sq.Cursor]:
    db_conn = sq.connect(filename)
    cursor = db_conn.cursor()
    return db_conn, cursor


def close_db(connection: sq.Connection):
    connection.commit()
    connection.close()


def create_top250_table(cursor: sq.Cursor):
...
def create_ratings_table(cursor: sq.Cursor):
...
def put_top_250_in_db(data_to_add: list[tuple], db_cursor: sq.Cursor):
